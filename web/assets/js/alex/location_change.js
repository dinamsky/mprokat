const addressChange = (function($) {
    ...
    function _changeCountry() {
        ...
        $.ajax({
            ...
            success: function(response){
                ui.$regionSelect.html(response).selectric('refresh');
                ui.$regionSelect.prop('selectedIndex', 0).selectric('refresh');
                ui.$citySelect.prop('selectedIndex', 0).prop('disabled', true).selectric('refresh');
            }
        });
    }
    ...
    function _changeRegion() {
        ...
        $.ajax({
            ...
            data: data,
            success: function(response){
                ui.$citySelect.html(response).selectric('refresh');
                ui.$citySelect.prop('selectedIndex', 0)
            }
        });

        ui.$citySelect.prop('selectedIndex', 0).selectric('refresh')
