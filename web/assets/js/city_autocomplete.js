$( document ).ready(function() {
    ...
    var my_city_autoComplete_main = new autoComplete({
        ...
        renderItem: function (item, search){
            ...
            search = search.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&');
            var re = new RegExp("(" + search.split(' ').join('|') + ")", "gi");
            return '<div class="autocomplete-suggestion city_block_main" data-url="' + res[2] + '" data-header="' + item + '" data-id="' + id + '" data-val = "' + id + '" >' + item.replace(re, "<b>$1</b>") + '</div>';
        },
        onSelect: function(e, term, item){
            $('.city_block_main[data-id="'+item.getAttribute('data-id')+'"]').click();
            // $('.city_block[data-id="'+term+'"]').click();
