...
class SitemapAuto extends Controller
{

    /**
     * @Route("/sitemap/sitemap-auto.xml", name="sitemap-auto", defaults={"_format"="xml"})
     */
    public function showAction(Request $request, MenuMarkModel $mm)
    {
        ...
        foreach ($em->getRepository('AppBundle:City')->findBy(['country'=>'RUS']) as $city) {
            ...
            foreach ($em->getRepository('AppBundle:Mark')->findAll() as $mark) {
                $urls[] = array(
                    'loc' => $this->generateUrl('search', array('city' => $city, 'service' => 'all', 'general' => 'auto','mark' => $mark->getHeader()))
                );
            }
        }
